...
class Save extends Action
{
    ...
    public function execute()
    {
		...
		$reservedAmount = $data['starting_price'];
			$productPrice = $data['max_price'];
                ...
                if (!$this->checkExist($pId)) {
                    ...
                    if ($data['max_price'] != null && $data['max_price'] != 0) {
                        if ($data['starting_price'] >= $data['max_price']) {
                            $this->messageManager->addErrorMessage('Reserved Price must be less than Starting Price');
                            return $resultRedirect->setPath($path, $params);
                        }
                    }
    ...
    protected function validateData($data) {
        if($data['sellproduct'] == 'yes') {
            if ($data['starting_price'] > $data['product_price']) {
                $this->messageManager->addErrorMessage(
                    'Reserved Price must be less than original product price'
                );
                return false;
            }
          ...
          //  if ($data['max_price'] > $data['product_price']) {
             //   $this->messageManager->addErrorMessage(
             //       'Starting Bidding price must be less than original product price'
             //   );
             //   return false;
           // }
            ...
            if ($data['max_price'] != null && $data['max_price'] != 0) {
                if ($data['starting_price'] > $data['max_price']) {
                    $this->messageManager->addErrorMessage('Reserved price must be less than starting bid');
                    return false;
                }
            }
        }
